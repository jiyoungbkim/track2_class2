...
<script type="text/javascript" language="javascript">
	$(document).ready(function() { //제이쿼리 실행
		$("#id").keyup(function() { //아이디id인 input에 입력하면 실행
			$("#ajaxJoin").empty();  
			if($('#id').val() != ""){
				var urlLocation = "/IdCheck_ajax"; //어떤 경로의 파일을 부를것인가 변수 선언
				var params = "t_id=" + $('#id').val(); //아이디가 id인 값을 t_id변수에 담아놓는다
				//ajax 실행되는 부분
				$.ajax({
					type : "POST",
					url : urlLocation,
					data : params,
					dataType : "text", //"/IdCheck2"을 실행한 결과의 리턴타입을 텍스트로 받겠다
					error : function() {
						alert('통신실패!!');
					},
					success : function(data) {
						...
						if(data_1 == "사용가능한 아이디입니다."){
							$("#ajaxJoin").css("display","inline-block");
							$("#ajaxJoin").css("color","green");
							//$("#ajaxJoin_2").css("display","none");
							$("#ajaxJoin").html(data); //html안의 내용이 리턴
						} else {
							$("#ajaxJoin").css("display","inline-block");
							$("#ajaxJoin").css("color","red");
							//$("#ajaxJoin_1").css("display","none");
							$("#ajaxJoin").html(data); //html안의 내용이 리턴
						}						
					}					
				});				
				...
				<form name="admin">
					<fieldset>
						...
						<label for="id" class="sr-only">아이디입력</label>
							...
							&nbsp;<span id="ajaxJoin"></span>										
							<!-- <span id="ajaxJoin_1"></span>
							<span id="ajaxJoin_2"></span>	 -->			
						
						<!-- <input type="button" value="중복검사" id="id_check"> -->
