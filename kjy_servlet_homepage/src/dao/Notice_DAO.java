...
public class Notice_DAO {
	...
	public String getMaxNo() {
		String query = " select max(notice_no) from a20_track2_sv_notice ";
		String result = "";
		try{
	...
	//게시물 번호 구하기 N0001
	public String getNoticeNo2() {
		String boardNo = getMaxNo();
		int plusNo = 0;
		
		if(boardNo == null) {
			boardNo = "N"+"0001";
		} else {
			String maxNo = boardNo.substring(2);
			
			plusNo = Integer.parseInt(maxNo);
			plusNo += 1;
			
			String r = CommonUtil.getLPad(Integer.toString(plusNo), 4, "0");			
			boardNo = "N"+r;
		}
		return boardNo;
	}
	//게시물 갯수 구하기
	public int getCount(){
		String query = " select count(*) from a20_track2_sv_notice ";
		int result = 0;
		try{
			con = common.getConnection();
			ps  = con.prepareStatement(query);
	...
	public int insertNotice(String notice_no,String title,String content,String reg_id,String reg_date) {
		String query = "insert into a20_track2_sv_notice(notice_no, title, content, reg_id, reg_date) "+
	...
	public int insertNotice(Notice_DTO dto) {
		...
		String query = "insert into a20_track2_sv_notice(notice_no, title, content, file_name_1, reg_id, reg_date, hit) "+
	...
	public ArrayList<Notice_DTO> getNoticeList(String selValue, String txtValue){
		...
		String query = " select n.notice_no, n.title, n.content, n.file_name_1, m.name, to_char(n.reg_date, 'yyyy-mm-dd'), n.hit "+
					" from a20_track2_sv_notice n, a20_track2_sv_member m "+
					" where n.reg_id = m.id "+
	...
	public ArrayList<Notice_DTO> getNoticeIndex(){
		ArrayList<Notice_DTO> arrN = new ArrayList<Notice_DTO>();
		String query = " select notice_no, substr(title, 1,60), to_char(reg_date, 'yyyy-mm-dd') "+
						" from (select notice_no, title, content, reg_date "+
						" from a20_track2_sv_notice "+
		...
		try{
			...
			while(rs.next()){
				...
				Notice_DTO dto = new Notice_DTO(notice_no,title,reg_date);
				arrN.add(dto);
			}
		} catch(RemoteException me) {
	...
	public int noticeHit(String noticeNo) {
		int result = 0;
		String query=" update a20_track2_sv_notice "+
					" set hit = hit+1 "+
	...
	public int updateNotice(Notice_DTO dto) {
		...
		if(!dto.getFile_name_1().equals("null")){	
			query = " update a20_track2_sv_notice "+
			...
			query = " update a20_track2_sv_notice "+
					" set title = '"+dto.getTitle()+"', content = '"+dto.getContent()+"', "+
					" reg_id = '"+dto.getReg_id()+"', reg_date = '"+dto.getReg_date()+"', file_name_1 = null "+
					" where notice_no = '"+dto.getNotice_no()+"' ";
		}
		int result = 0;
		try {
	...
	public int deleteNotice(String notice_no) {
		...
		String query = " delete from a20_track2_sv_notice "+
						" where notice_no = '"+notice_no+"' ";
		int result = 0;
		try {
			con = common.getConnection();
